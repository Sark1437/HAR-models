# Description: HAR model - Out-of-sample forecast
# Course: SQB7002
...
for (rw in points){
  ...
  x1 <- c(RV[(rw):(rw+window-2)])
  x2 <- c(RV.w[(rw):(rw+window-2)])
  x3 <- c(RV.m[(rw):(rw+window-2)])
  X <- cbind(1,x1,x2,x3)
...
#   rw.RV.hat.HAR[rw+window] <- rw.X[[rw]]%*%rw.results[[rw]]
# }
#ALT Prediction
rw.RV.hat.HAR <-c(0)
for (rw in points){
  rw.RV.hat.HAR[rw+window] <- (rw.results[[rw]][1]+rw.results[[rw]][2]*RV[rw+window-1]+rw.results[[rw]][3]*mean(RV[(window-1-4+rw):(window-1+rw)])
                       +rw.results[[rw]][4]*mean(RV[(rw+window-1-21):(window-1+rw)]))
}
#Residuals
