...
# library(optextras)
# optsp$deps <- 1e-2
# grad_forward <- function(v){grcentral(v,Like,env = optsp)}
# start <- Sys.time()
# result_SHARK_BFGS <- suppressWarnings(optim(c(-0.0114,-0.0101,0.0127,-0.050,-0.0229),gr = function(...) grad_forward(...),Like, method = "BFGS", hessian = TRUE))
# (result_SHARK_BFGS$time <- Sys.time()-start)

start <- Sys.time()
result_SHARK_BFGS_full <- optim(c(-0.0102,-0.0130,0.0147,-0.0062,-0.0224),Like, method = "BFGS", control = list() , hessian = TRUE)
...
lines(RV.hat.SHARK[1:length(RV)],col="red")
...
#Residuals
res.SHARK <- RV[3897:4096]-RV.hat.SHARK[3897:4096]
(MSE.SHARK <- mean(res.SHARK^2)) 
(QLIKE.SHARK <- mean(RV[3897:4096]/RV.hat.SHARK[3897:4096]-log(RV[3897:4096]/RV.hat.SHARK[3897:4096])-1))

#Plots
plot(RV[3897:4096],type="l",xlab="Observations",ylab="RV")
title("S&P500 (from 1997-04-08 to 2013-08-30)")
lines(RV.hat.SHARK[3897:4096],col="red")
...
var95 <- -1.52*sqrt(RV.hat.SHARK[3897:4096]) #0.05 quantile of standardized t with dof=4.17
